...
impl CryptoEndpoints {
    ...
    pub async fn exchange_rate(&self, from_currency: &str, to_currency: &str) -> Result<CryptoExchangeRate> {
    ...
    pub async fn health_index(&self, symbol: &str, market: &str) -> Result<CryptoHealthIndex> {
        ...
        let volumes: Vec<f64> = data.time_series.values()
            .take(30)
            .filter_map(|price| price.volume.parse().ok())
            .collect();
