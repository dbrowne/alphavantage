...
use av_models::news::{NewsSentiment, NewsArticle, SentimentLabel};
...
async fn analyze_market_sentiment(client: &AlphaVantageClient) -> Result<(), Error> {
    ...
    let news = client.news().news_sentiment_with_options(
        Some(vec!["financial_markets".to_string(), "stock_market".to_string()]),
        None,
        None,
        None,
        Some("LATEST"),
...
async fn analyze_sector_sentiment(
    ...
    let news = client.news().news_sentiment_by_tickers(ticker_strings.clone(), 80).await?;
...
async fn analyze_earnings_news(client: &AlphaVantageClient) -> Result<(), Error> {
    ...
    let news = client.news().news_sentiment_by_topics(
        vec!["earnings".to_string(), "quarterly_results".to_string()],
        60,
    ).await?;
    ...
    println!("\n💡 Earnings Insights:");
    let earnings_articles: Vec<_> = news.feed.iter()
        .filter(|article| {
...
async fn analyze_crypto_sentiment(client: &AlphaVantageClient) -> Result<(), Error> {
    ...
    let news = client.news().news_sentiment_with_options(
        Some(vec!["cryptocurrency".to_string(), "bitcoin".to_string(), "blockchain".to_string()]),
        None,
        None,
        None,
        Some("LATEST"),
    ...
    for (i, topic) in analysis.topic_analysis.iter().take(5).enumerate() {
        ...
        println!("  {}. {} {} (relevance: {:.2}, sentiment: {:.3})", 
                 i + 1,
                 sentiment_emoji,
                 topic.topic,
                 topic.relevance,
        ...
        if !new_articles.is_empty() {
            ...
            for article in &new_articles {
                ...
                if !article.ticker_sentiment.is_empty() {
                    let tickers: Vec<_> = article.ticker_sentiment.iter()
                        .take(3)
                        .map(|ts| &ts.ticker)
                        .collect();
...
fn analyze_sentiment(news: &NewsSentiment, tickers: Option<Vec<String>>) -> SentimentAnalysis {
    ...
    if let Some(ticker_list) = tickers {
        for ticker in ticker_list {
            ...
            if !sentiments.is_empty() {
                ...
                ticker_sentiments.insert(ticker.clone(), TickerSentimentSummary {
                    ticker: ticker.clone(),
    ...
    let recent_headlines: Vec<Headline> = news.feed.iter()
        .take(10)
