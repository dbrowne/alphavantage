...
async fn analyze_portfolio(
    ...
    for holding in portfolio {
        ...
        let current_price = quote.global_quote.price_as_f64()
...
async fn get_quote_with_retry(
    ...
    for attempt in 1..=3 {
        match client.time_series().daily(symbol).await {
            Ok(daily_data) => {
...
async fn get_sentiment_with_retry(
    client: &AlphaVantageClient,
    symbol: &str,
) -> Result<f64, Error> {
    for attempt in 1..=3 {
        match client.news().news_sentiment_by_tickers(vec![symbol.to_string()], 20).await {
            Ok(news) => {
...
fn display_portfolio_analysis(analysis: &PortfolioAnalysis) {
    ...
    for holding in &analysis.holdings_analysis {
        ...
        let pe_str = holding.pe_ratio
            .map(|pe| format!("{:.1}", pe))
            .unwrap_or_else(|| "N/A".to_string());
        
        println!("│ {:<7} │ {:<7.1} │ ${:<6.2} │ ${:<6.0} │ {}{:<6.0} │ {:+<6.1}% │ {:<6.1}% │ {:<7} │",
                 holding.symbol,
                 holding.shares,
    ...
    match client.fundamentals().top_gainers_losers().await {
        Ok(top_movers) => {
            ...
            for (i, gainer) in top_movers.top_gainers.iter().take(3).enumerate() {
                println!("  {}. {} - ${} ({})", 
                         i + 1, gainer.ticker, gainer.price, gainer.change_percentage);
            }
            
            println!("\n📉 Top 3 Losers:");
            for (i, loser) in top_movers.top_losers.iter().take(3).enumerate() {
                println!("  {}. {} - ${} ({})", 
                         i + 1, loser.ticker, loser.price, loser.change_percentage);
            }
        }
        Err(e) => {
...
async fn analyze_portfolio_sentiment(
    ...
    match client.news().news_sentiment_by_tickers(symbols, 50).await {
        Ok(news) => {
    ...
    match client.fundamentals().top_gainers_losers().await {
        Ok(top_movers) => {
            ...
            for (i, active) in top_movers.most_actively_traded.iter().take(5).enumerate() {
                println!("  {}. {} - ${} (Vol: {})", 
                         i + 1, active.ticker, active.price, active.volume);
            }
        }
        Err(e) => {
