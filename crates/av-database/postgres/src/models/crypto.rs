...
use crate::schema::{crypto_overview_basic, crypto_overview_metrics, crypto_technical, crypto_social,crypto_api_map};
...
impl CryptoApiMap {
    ...
    pub fn get_stale_mappings(
        ...
        crypto_api_map
            ...
            .filter(is_active.eq(Some(true)))
            .filter(
                last_verified.is_null()
                    .or(last_verified.lt(threshold_date))
            )
            .load::<Self>(conn)
    }
