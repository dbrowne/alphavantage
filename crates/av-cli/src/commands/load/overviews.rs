...
fn get_symbols_to_load(
    ...
    if let Some(ref symbol_list) = symbols_arg {
        ...
        let file_symbols: Vec<String> = content
            .lines()
        ...
        if let Some(limit_val) = limit {
            info!("Found {} US equity symbols without overviews (limited to {})",
                  symbol_records.len(), limit_val);
        } else {
...
fn save_overviews_to_db(
    ...
    use av_database_postgres::schema::{overviews, overviewexts, symbols};
    use diesel::PgConnection;
    ...
    conn.transaction::<_, diesel::result::Error, _>(|conn| {
        for overview_data in data {
            ...
            let new_overview = NewOverviewOwned {
                ...
                market_capitalization: parse_i64(&overview_data.overview.market_capitalization).unwrap_or(0),
            ...
            let new_overview_ext = NewOverviewextOwned {
                sid: overview_data.sid,
                revenue_per_share_ttm: parse_f32(&overview_data.overview.revenue_per_share_ttm).unwrap_or(0.0),
                profit_margin: parse_f32(&overview_data.overview.profit_margin).unwrap_or(0.0),
                operating_margin_ttm: parse_f32(&overview_data.overview.operating_margin_ttm).unwrap_or(0.0),
                return_on_assets_ttm: parse_f32(&overview_data.overview.return_on_assets_ttm).unwrap_or(0.0),
                return_on_equity_ttm: parse_f32(&overview_data.overview.return_on_equity_ttm).unwrap_or(0.0),
                ...
                quarterly_earnings_growth_yoy: parse_f32(&overview_data.overview.quarterly_earnings_growth_yoy).unwrap_or(0.0),
                quarterly_revenue_growth_yoy: parse_f32(&overview_data.overview.quarterly_revenue_growth_yoy).unwrap_or(0.0),
                analyst_target_price: parse_f32(&overview_data.overview.analyst_target_price).unwrap_or(0.0),
                ...
                price_to_sales_ratio_ttm: parse_f32(&overview_data.overview.price_to_sales_ratio_ttm).unwrap_or(0.0),
                ...
                day_moving_average_50: parse_f32(&overview_data.overview.day_50_moving_average).unwrap_or(0.0),
                day_moving_average_200: parse_f32(&overview_data.overview.day_200_moving_average).unwrap_or(0.0),
