...
pub async fn execute(args: CryptoSocialArgs, config: Config) -> Result<()> {
  ...
  let crypto_symbols =
    load_crypto_symbols_from_db(&config.database_url, &args.symbols_list, args.limit)
  ...
  let input =
    CryptoSocialInput { symbols: Some(crypto_symbols), update_existing: args.update_existing };
  ...
  let social_data =
    social_loader.load_data(&input, &loader_context).await.context("Failed to load social data")?;
  ...
  if !args.dry_run && !social_data.is_empty() {
    ...
    let (inserted, updated) =
      save_social_data_to_db(&config.database_url, &social_data, args.update_existing)
        .await
...
fn load_crypto_symbols_from_db(
  ...
  let mut query = symbols_table
    .left_join(api_map_table.on(sid.eq(crypto_api_map::sid).and(api_source.eq("CoinGecko"))))
  ...
  let results: Vec<(i64, String, String, Option<String>)> =
