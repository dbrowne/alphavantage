...
use av_loaders::{
    NewsLoaderConfig, LoaderConfig, LoaderContext,
...
fn get_specific_crypto_symbols(database_url: &str, symbol_list: &[String]) -> Result<Vec<SymbolInfo>> {
    use diesel::prelude::*;
    use av_database_postgres::schema::symbols;

    let mut conn = PgConnection::establish(database_url)?;

    let symbols_upper: Vec<String> = symbol_list.iter()
