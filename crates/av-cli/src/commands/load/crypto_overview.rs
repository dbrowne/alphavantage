...
pub async fn execute(args: CryptoOverviewArgs, config: Config) -> Result<()> {
    ...
    for (sid, symbol, name) in symbols_to_load {
        ...
        match fetch_crypto_overview(&client, sid, &symbol, &name).await {
            Ok(overview) => {
                ...
                let github_data = if args.include_github {
                    if let Some(ref github_url) = overview.github {
                        ...
                        let gh_data = fetch_github_data(&client, Some(github_url), args.github_token.as_ref()).await;

                        if let Some(ref gh) = gh_data {
                            info!("GitHub stats for {}: {} stars, {} forks, {} contributors",
                                  symbol, gh.stars.unwrap_or(0), gh.forks.unwrap_or(0), gh.contributors.unwrap_or(0));
                        }
                ...
                info!("Successfully loaded overview for {}: Market Cap ${}, Rank #{}",
...
pub async fn update_github_data(args: UpdateGitHubArgs, config: Config) -> Result<()> {
    ...
    let cryptos_with_github = tokio::task::spawn_blocking({
        ...
        let limit = args.limit;
        move || {
            use av_database_postgres::schema::{symbols, crypto_social};
    ...
    for (sid, symbol, github_url) in cryptos_with_github {
        ...
        if let Some(github_data) = fetch_github_data(&client, github_url.as_ref(), args.github_token.as_ref()).await {
            ...
            if update_result.is_ok() {
                info!("Updated GitHub data for {}: {} stars, {} forks",
                      symbol, github_data.stars.unwrap_or(0), github_data.forks.unwrap_or(0));
                updated_count += 1;
            }
        }
    ...
    progress.finish_with_message(format!("Updated GitHub data for {} cryptocurrencies", updated_count));
    Ok(())
}
...
fn get_crypto_symbols_to_load(
    ...
    if results.is_empty() && specific_symbols.is_some() {
        warn!("No cryptocurrency symbols found that need overviews (or they might not be type 'Cryptocurrency')");
    }
...
async fn fetch_from_coinpaprika(
    ...
    Ok(CryptoOverviewData {
        ...
        market_cap: quotes["market_cap"]
            .as_f64()
            .unwrap_or(0.0) as i64,
        circulating_supply: ticker_data["circulating_supply"]
            .as_f64()
            .unwrap_or(0.0),
        ...
        volume_24h: quotes["volume_24h"]
            .as_f64()
            .unwrap_or(0.0) as i64,
        price_change_24h: quotes["percent_change_24h"]
            .as_f64()
            .unwrap_or(0.0),
...
async fn fetch_github_data(
    ...
    let contributors_url = format!("https://api.github.com/repos/{}/{}/contributors?per_page=1", owner, repo);
    let mut contributors_request = client
        .get(&contributors_url)
        .header("Accept", "application/vnd.github.v3+json");

    if let Some(token) = github_token {
        contributors_request = contributors_request.header("Authorization", format!("Bearer {}", token));
    }
    ...
    let mut commits_request = client
        .get(&commits_url)
        .header("Accept", "application/vnd.github.v3+json");
...
fn save_crypto_overviews_with_github_to_db(
    ...
    conn.transaction::<_, anyhow::Error, _>(|conn| {
        for (overview, github_data) in overviews {
            ...
            let circulating_supply_bd = BigDecimal::from_str(&overview.circulating_supply.to_string()).ok();
            let total_supply_bd = overview.total_supply
                .and_then(|ts| BigDecimal::from_str(&ts.to_string()).ok());
            let max_supply_bd = overview.max_supply
                .and_then(|ms| BigDecimal::from_str(&ms.to_string()).ok());
        ...
        diesel::insert_into(crypto_overview_metrics::table)
            ...
            let mut new_technical = NewCryptoTechnical {
                ...
                is_defi: false,
                is_stablecoin: overview.symbol.contains("USD") || overview.symbol.contains("USDT") || overview.symbol.contains("USDC"),
                is_nft_platform: false,
                is_exchange_token: overview.symbol == "BNB" || overview.symbol == "CRO" || overview.symbol == "OKB",
                is_gaming: false,
                is_metaverse: false,
