...
fn process_social(
    ...
    if website_url.is_some() || whitepaper_url.is_some() || github_url.is_some() ||
        twitter_handle.is_some() || telegram_url.is_some() || reddit_url.is_some() {
...
fn process_technical(
    ...
    let is_defi = categories.iter().any(|c|
        c.contains("defi") || c.contains("decentralized-finance"));

    let is_stablecoin = categories.iter().any(|c|
        c.contains("stablecoin") || c.contains("stable"));

    let is_nft_platform = categories.iter().any(|c|
        c.contains("nft") || c.contains("non-fungible"));

    let is_exchange_token = categories.iter().any(|c|
        c.contains("exchange") || c.contains("cex-token") || c.contains("dex-token"));

    let is_gaming = categories.iter().any(|c|
        c.contains("gaming") || c.contains("game"));

    let is_metaverse = categories.iter().any(|c|
        c.contains("metaverse") || c.contains("virtual"));

    let is_privacy_coin = categories.iter().any(|c|
        c.contains("privacy"));

    let is_layer2 = categories.iter().any(|c|
        c.contains("layer-2") || c.contains("l2"));

    let is_wrapped = categories.iter().any(|c|
        c.contains("wrapped"));

    // Extract developer data
    let developer_data = data.get("developer_data");

    let github_forks = developer_data
        .and_then(|dd| dd.get("forks"))
        .and_then(|v| v.as_i64())
        .map(|v| v as i32);

    let github_stars = developer_data
        .and_then(|dd| dd.get("stars"))
        .and_then(|v| v.as_i64())
        .map(|v| v as i32);
...
fn determine_blockchain_platform(data: &Value, categories: &[String]) -> Option<String> {
    ...
    if let Some(blockchain_sites) = data
        .get("links")
        .and_then(|l| l.get("blockchain_site"))
