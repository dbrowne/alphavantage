...
use diesel::{sql_query, prelude::*, sql_types};
...
impl CryptoMetadataLoader {
    ...
    async fn load_alphavantage_metadata_cached(
        ...
        if !self.config.force_refresh {
            if let Some(cached_data) = self.get_cached_response(database_url, &cache_key, "alphavantage").await {
    ...
    async fn load_alphavantage_metadata_fresh(
        ...
        let api_key = self.config.alphavantage_api_key.as_ref()
            .ok_or_else(|| CryptoLoaderError::ApiError("AlphaVantage API key not provided".to_string()))?;
        ...
        if !response.status().is_success() {
            return Err(CryptoLoaderError::ApiError(
                format!("AlphaVantage API returned status: {}", response.status())
            ));
        }

        let response_text = response.text().await
            .map_err(|e| CryptoLoaderError::ApiError(format!("Failed to read AlphaVantage response: {}", e)))?;

        // Parse response to extract metadata
        let crypto_daily: CryptoDaily = serde_json::from_str(&response_text)
            .map_err(|e| CryptoLoaderError::ParseError(format!("Failed to parse AlphaVantage response: {}", e)))?;
    ...
    fn process_alphavantage_response(
        ...
        additional_data.insert("alphavantage_info".to_string(),
                               serde_json::to_value(&crypto_daily.meta_data)
        ...
        Ok(ProcessedCryptoMetadata {
            ...
            additional_data: Some(serde_json::to_value(additional_data).unwrap_or(serde_json::Value::Null)),
    ...
    async fn load_coingecko_metadata_cached(
        ...
        // Try cache first (unless force refresh is enabled)
        if !self.config.force_refresh {
            if let Some(cached_data) = self.get_cached_response(database_url, &cache_key, "coingecko").await {
    ...
    async fn load_coingecko_metadata_fresh(
        ...
        let coin_data: Value = response.json().await
            .map_err(|e| CryptoLoaderError::ParseError(format!("Failed to parse CoinGecko response: {}", e)))?;
    ...
    fn process_coingecko_response(
        ...
        Ok(ProcessedCryptoMetadata {
            ...
            is_active: symbol.is_active,
            additional_data: if additional_data.is_empty() { None } else { Some(serde_json::to_value(additional_data)?) },
            last_updated: Utc::now(),
        })
    }
                ...
                let result = match source {
                    ...
                    _ => {
                        ...
                        if self.config.alphavantage_api_key.is_some() && source != CryptoDataSource::CoinGecko {
                            ...
                            Err(CryptoLoaderError::ApiError(format!("Source {:?} not supported for metadata", source)))
                        }
                    }
                };
                ...
                match result {
                    ...
                    Err(e) => {
                        ...
                        if error_msg.contains("rate limit") || error_msg.contains("429") {
                            rate_limited = true;
                            warn!("Rate limited for {}, attempt {}/{}", symbol.symbol, attempts, self.config.max_retries);

                            // Exponential backoff for rate limiting
                            let backoff_delay = self.config.delay_ms * 2_u64.pow(attempts as u32);
                            sleep(Duration::from_millis(backoff_delay)).await;
                        } else {
                            error!("Failed to load metadata for {} (attempt {}/{}): {}",
...
impl DataLoader for CryptoMetadataLoader {
    ...
    async fn load(&self, context: &LoaderContext, input: Self::Input) -> LoaderResult<Self::Output> {
        ...
        info!("Starting crypto metadata loader with caching enabled: {}", self.config.enable_response_cache);

        if let Some(tracker) = &context.process_tracker {
            tracker.start("crypto_metadata_loader").await
        ...
        info!("Processing metadata for {} symbols from {} sources (cache TTL: {}h)",
              symbols_count, input.sources.len(), self.config.cache_ttl_hours);
        ...
        for source in input.sources {
            ...
            for batch in symbols.chunks(self.config.batch_size) {
                let (metadata, result) = self.process_batch(context, batch.to_vec(), source, &database_url).await;
            ...
            info!("Completed {:?}: {} processed, {} failed, caching: {}",
                  source, combined_result.symbols_processed, combined_result.symbols_failed,
        ...
        info!("Crypto metadata loader completed in {}ms: {} processed, {} failed, caching: {}",
              processing_time, all_metadata.len(), total_failed,
              if self.config.enable_response_cache { "enabled" } else { "disabled" });
